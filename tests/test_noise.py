...
class TestNoise:
    ...
    def test_no_noise(self, simple_geometry):
        ...
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.0)
    ...
    def test_basic_noise(self, simple_geometry):
        """基本的なノイズテスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.1)
        assert isinstance(result, Geometry)
        # ノイズが適用されるので座標が変化
    ...
    def test_max_noise(self, simple_geometry):
        ...
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=1.0)
        assert isinstance(result, Geometry)
    ...
    def test_various_intensities(self, simple_geometry):
        ...
        for intensity in [0.1, 0.3, 0.5, 0.7, 0.9]:
            result = effect.apply_to_geometry(simple_geometry, intensity=intensity)
            assert isinstance(result, Geometry)
    ...
    def test_frequency_single_value(self, simple_geometry):
        """単一値周波数テスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5, frequency=0.3)
        assert isinstance(result, Geometry)

    def test_frequency_tuple(self, simple_geometry):
        """タプル周波数テスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5, frequency=(0.2, 0.4, 0.6))
        assert isinstance(result, Geometry)
    ...
    def test_time_parameter(self, simple_geometry):
        ...
        result1 = effect.apply_to_geometry(simple_geometry, intensity=0.5, t=0.0)
        result2 = effect.apply_to_geometry(simple_geometry, intensity=0.5, t=1.0)
    ...
    def test_reproducibility(self, simple_geometry):
        ...
        # 同じパラメータで複数回実行
        result1 = effect.apply_to_geometry(simple_geometry, intensity=0.5, frequency=0.3, t=0.5)
        result2 = effect.apply_to_geometry(simple_geometry, intensity=0.5, frequency=0.3, t=0.5)
    ...
    def test_high_frequency(self, simple_geometry):
        """高周波数テスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5, frequency=1.0)
        assert isinstance(result, Geometry)

    def test_low_frequency(self, simple_geometry):
        """低周波数テスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5, frequency=0.0)
        assert isinstance(result, Geometry)

    def test_mixed_frequency_tuple(self, simple_geometry):
        """混合周波数タプルテスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5, frequency=(0.0, 0.5, 1.0))
        assert isinstance(result, Geometry)

    def test_negative_time(self, simple_geometry):
        """負の時間テスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5, t=-1.0)
        assert isinstance(result, Geometry)

    def test_large_time(self, simple_geometry):
        """大きな時間値テスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5, t=1000.0)
        assert isinstance(result, Geometry)

    def test_preserves_structure(self, simple_geometry):
        """構造保持テスト"""
        effect = Noise()
        result = effect.apply_to_geometry(simple_geometry, intensity=0.5)
        assert isinstance(result, Geometry)
