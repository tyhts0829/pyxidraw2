...
def draw(t, cc) -> Geometry:
    ...
    circle = np.array(
        [[center_x + radius * np.cos(angle), center_y + radius * np.sin(angle), 0] for angle in angles],
        dtype=np.float32,
    )

    # 対角線
    diagonal1 = np.array([[50, 50, 0], [150, 150, 0]], dtype=np.float32)

    diagonal2 = np.array([[150, 50, 0], [50, 150, 0]], dtype=np.float32)
